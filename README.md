...
To set up Google Play Services version, you can use PLAY_SERVICES_AUTH_VERSION and PLAY_SERVICES_IDENTITY_VERSION parameters (with 15.0.1 value by default). It is useful in order to avoid conflicts with another plugins which use any other different version of Google Play Service, because they MUST be the same version.
